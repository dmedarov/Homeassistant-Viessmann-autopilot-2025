...
# VIESSMANN AUTOPILOT 2040.04 • RADIATOR 57.9°C RELIGION EDITION – FINAL & FLAWLESS
# 140 m² • радиатори • никога над 57.9°C • перфектен лог завинаги

logger.info("═" * 130)
logger.info("VIESSMANN AUTOPILOT 2040.04 • RADIATOR 57.9°C RELIGION • ЗАПУСНАТ УСПЕШНО")
logger.info("═" * 130)
...
try:
    ...
    if 7 <= hour <= 18 and outdoor > -10 and pv_now > 0.7:
        mult = 1.85 if night_tariff else 1.65
        extra = min(pv_rem * 0.38, 3.4) if hour < 14 and pv_rem > 5 and pv_now > 2.2 else 0.0
        raw_pv_boost = min(pv_now * mult + extra, 6.8)
    smoothing = 0.30 if (pv_rem + pv_now/12) >= 5 else 0.18
    pv_boost = round(prev_pv_boost + smoothing * (raw_pv_boost - prev_pv_boost), 1)
    pv_boost = max(min(pv_boost, 6.8), 0.0)
    pv_boost = min(pv_boost, max(0.0, 57.9 - supply_temp - 0.8))
    hass.states.set('sensor.pv_boost_smoothed', pv_boost, {'icon': 'mdi:solar-power-variant'})

    final_shift = pi_shift = pi_shift * 0.62 + zone_shift * 0.22 + pv_boost * 0.10
    ...
    hass.states.set('sensor.autopilot_persistent', 0.0, {
        'last_slope': slope, 'last_shift': final_shift,
        'friendly_name': 'Autopilot 2040.04 RADIATOR 57.9°C RELIGION'
    })
    ...
    # ПЕРФЕКТНИЯТ РЕД ЗА ЛОГ – НИКОГА ПОВЕЧЕ НЯМА ДА СЕ ПИПА
    log_line = f"{day:02d}.{month:02d} {hour:02d}:{minute:02d} │ In {indoor:5.1f}→{target:5.1f} │ Out {outdoor:5.1f}({tmin_72h:+5.0f}/{tmin_96h:+5.0f}) │ Sh {final_shift:+6.1f} │ Sl {slope:6.3f} │ PV {pv_boost:6.1f} │ Supp {supply_temp:6.1f}°C │ PI {pi_shift:+6.1f} Z{zone_shift:5.1f} │ {mode_str}"

    hass.states.set('sensor.autopilot_log_last_line', log_line)
    logger.info(log_line)

    hass.states.set(FAIL_ENTITY, 0)
    logger.info("AUTOPILOT 2040.04 • РАБОТИ КАТО ШВЕЙЦАРСКИ ЧАСОВНИК")
    logger.info("═" * 130)
    ...
    if fail_count >= 4:
        ...
        hass.services.call('notify', 'mobile_app_damian_iphone', {'message': 'АВТОПИЛОТ 2040.04 → SAFE MODE'})
e': 'АВТОПИЛОТ → SAFE MODE'})
